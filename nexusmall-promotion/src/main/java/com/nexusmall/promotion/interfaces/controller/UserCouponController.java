...
@ApiVersion("v1")
@Tag(name = "用户优惠券管�?, description = "查询、使用、锁定、释放用户优惠券")
public class UserCouponController {
     ...
     *
     * @param userId    用户ID
     * @param useStatus 使用状态（0-未使�?1-已使�?2-已过�?3-已锁定，null表示全部�?
     * @return 优惠券列�?
     */
    @GetMapping(value = "/", headers = "X-API-Version=v1")
    @Operation(summary = "查询用户优惠券列�?, description = "支持按状态筛选：未使�?已使�?已过�?已锁�?)
    public Result<List<UserCouponVO>> listUserCoupons(
            @Parameter(description = "用户ID", required = true)
            @RequestHeader("X-User-ID") Long userId,
            @Parameter(description = "使用状态（0-未使�?1-已使�?2-已过�?3-已锁定，null表示全部�?)
    ...
    @Operation(summary = "核销优惠�?, description = "订单支付时调用，将优惠券标记为已使用")
    ...
    @Operation(summary = "锁定优惠�?, description = "创建订单但未支付时调用，防止优惠券被其他订单使用")
    ...
    @Operation(summary = "释放优惠�?, description = "订单取消或支付失败时调用，恢复优惠券为未使用状�?)
    ...
    @Operation(summary = "退款回退优惠�?, description = "订单退款时调用，如果优惠券未过期则恢复为未使用状�?)
